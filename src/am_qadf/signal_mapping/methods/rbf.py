"""
Radial Basis Functions (RBF) Interpolation - C++ Wrapper

Thin Python wrapper for C++ RBF interpolation implementation.
...
try:
    from am_qadf_native.signal_mapping import RBFMapper
    CPP_AVAILABLE = True
except ImportError:
    CPP_AVAILABLE = False
    RBFMapper = None
...
class RBFInterpolation(InterpolationMethod):
    """
    RBF interpolation - C++ wrapper.
    
    This is a thin wrapper around the C++ RBFMapper implementation.
    All core computation is done in C++.
    """
    ...
    def __init__(
        ...
        Args:
            kernel_type: RBF kernel type ('gaussian', 'multiquadric', 'thin_plate')
            epsilon: RBF shape parameter (default: 1.0)
        """
