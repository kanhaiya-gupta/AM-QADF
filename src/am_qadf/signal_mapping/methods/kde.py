"""
Gaussian Kernel Density Estimation (KDE) Interpolation - C++ Wrapper

Thin Python wrapper for C++ KDE interpolation implementation.
...
try:
    from am_qadf_native.signal_mapping import KDEMapper
    CPP_AVAILABLE = True
except ImportError:
    CPP_AVAILABLE = False
    KDEMapper = None
...
class GaussianKDEInterpolation(InterpolationMethod):
    """
    Gaussian KDE interpolation - C++ wrapper.
    
    This is a thin wrapper around the C++ KDEMapper implementation.
    All core computation is done in C++.
    """
    ...
    def __init__(self, bandwidth: Optional[float] = None, adaptive: bool = False):
        ...
        Args:
            ...
            adaptive: Whether to use adaptive bandwidth (not used in C++ wrapper yet)
        """
        ...
        self._mapper = KDEMapper(self.bandwidth, "gaussian")

    def interpolate(self, points: np.ndarray, signals: Dict[str, np.ndarray], voxel_grid: VoxelGrid) -> VoxelGrid:
        """
        Interpolate points to voxel grid using Gaussian KDE method.
