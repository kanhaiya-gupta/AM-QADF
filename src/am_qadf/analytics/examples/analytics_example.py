...
from ..sensitivity_analysis.global_analysis import (
...
class PBFProcessModel:
    ...
    def evaluate_quality(self, parameters: np.ndarray) -> float:
        ...
        quality = power_effect + speed_effect + hatch_effect + layer_effect + temp_effect + pressure_effect + noise
    ...
    def evaluate_porosity(self, parameters: np.ndarray) -> float:
        ...
        porosity = power_effect + speed_effect + hatch_effect + layer_effect + temp_effect + pressure_effect + noise
...
class AnalyticsExample:
    ...
    def _run_sensitivity_analysis(self) -> Dict[str, Any]:
        ...
        return {"sobol_analysis": sobol_result, "morris_analysis": morris_result}
    ...
    def _run_uncertainty_analysis(self) -> Dict[str, Any]:
        ...
        parameter_distributions = {
            ...
            "hatch_spacing": {
                "type": "uniform",
                "params": {"lower": 0.05, "upper": 0.15},
            },
            "layer_thickness": {
                "type": "uniform",
                "params": {"lower": 0.02, "upper": 0.08},
            },
            "preheat_temp": {"type": "normal", "params": {"mu": 120, "sigma": 30}},
            "atmosphere_pressure": {
                "type": "uniform",
                "params": {"lower": 0.1, "upper": 1.0},
            },
        }
        ...
        return {"monte_carlo_analysis": monte_carlo_result}
    ...
    def _run_statistical_analysis(self, process_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # PCA analysis
        pca_result = self.analyzers["pca"].analyze(data=process_data, feature_names=feature_names, n_components=3)
    ...
    def _run_time_series_analysis(self, process_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Trend analysis
        trend_result = self.analyzers["trend"].analyze(time_series=quality_series, method="linear")

        # Seasonality analysis
        seasonality_result = self.analyzers["time_series"].analyze_seasonality(quality_series)

        # Forecasting
        forecasting_result = self.analyzers["time_series"].analyze_forecasting(time_series=quality_series, forecast_horizon=10)
