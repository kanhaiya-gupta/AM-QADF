...
class CompletenessAnalyzer:
    ...
    def fill_gaps(
        ...
        if strategy == GapFillingStrategy.NONE:
            ...
            filled_array[missing_mask] = filled_array[
                indices[0][missing_mask],
                indices[1][missing_mask],
                indices[2][missing_mask],
            ]
        elif strategy == GapFillingStrategy.LINEAR:
            # Handle 1D arrays with np.interp, multi-dimensional with scipy
            if filled_array.ndim == 1:
                ...
                if len(valid_indices) >= 2:
                    ...
                    if len(missing_indices) > 0:
                        ...
                        filled_values = np.interp(missing_indices, valid_indices, valid_values)
                        filled_array[missing_indices] = filled_values
                elif len(valid_indices) == 1:
                    # Only one valid value, fill all with that value
                    filled_array[missing_mask] = filled_array[valid_indices[0]]
                else:
                    # No valid values, fill with 0
                    filled_array[missing_mask] = 0.0
            else:
                # Multi-dimensional: Use scipy interpolation
                ...
                if np.any(valid_mask):
                    ...
                    if len(missing_coords[0]) > 0:
                        ...
                        # Replace NaN with 0.0 if interpolation failed
                        filled_values = np.nan_to_num(filled_values, nan=0.0)
