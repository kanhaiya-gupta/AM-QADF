...
class GlobalSensitivityAnalyzer:
    ...
    def analyze_sobol(
        ...
        try:
            ...
            param_values = saltelli.sample(problem, self.config.sample_size, calc_second_order=True)

            # Evaluate model for all samples
            model_outputs = self._evaluate_model_parallel(model_function, param_values)

            # Ensure model_outputs is a 1D array
            model_outputs = np.asarray(model_outputs).flatten()
            ...
            # First order indices - handle both array and scalar cases
            s1_values = sobol_indices["S1"]
            if np.isscalar(s1_values):
                s1_values = np.array([s1_values])
            else:
                s1_values = np.asarray(s1_values).flatten()

            for i, name in enumerate(parameter_names):
                if i < len(s1_values):
                    sensitivity_indices[f"S1_{name}"] = float(s1_values[i])
                if "S1_conf" in sobol_indices:
                    s1_conf = sobol_indices["S1_conf"]
                    if not np.isscalar(s1_conf) and i < len(s1_conf):
                        if np.isscalar(s1_conf[i]):
            ...
            # Total order indices - handle both array and scalar cases
            st_values = sobol_indices["ST"]
            if np.isscalar(st_values):
                st_values = np.array([st_values])
            else:
                st_values = np.asarray(st_values).flatten()

            for i, name in enumerate(parameter_names):
                if i < len(st_values):
                    sensitivity_indices[f"ST_{name}"] = float(st_values[i])
                if "ST_conf" in sobol_indices:
                    st_conf = sobol_indices["ST_conf"]
                    if not np.isscalar(st_conf) and i < len(st_conf):
                        if np.isscalar(st_conf[i]):
    ...
    def analyze_morris(
        ...
        try:
            ...
            # Handle mu, sigma, mu_star - convert to arrays if needed
            mu_values = morris_indices["mu"]
            if np.isscalar(mu_values):
                mu_values = np.array([mu_values])
            else:
                mu_values = np.asarray(mu_values).flatten()

            sigma_values = morris_indices["sigma"]
            if np.isscalar(sigma_values):
                sigma_values = np.array([sigma_values])
            else:
                sigma_values = np.asarray(sigma_values).flatten()

            mu_star_values = morris_indices["mu_star"]
            if np.isscalar(mu_star_values):
                mu_star_values = np.array([mu_star_values])
            else:
                mu_star_values = np.asarray(mu_star_values).flatten()

            for i, name in enumerate(parameter_names):
                # Elementary effects mean (mu)
                if i < len(mu_values):
                    sensitivity_indices[f"mu_{name}"] = float(mu_values[i])
                if "mu_conf" in morris_indices:
                    mu_conf = morris_indices["mu_conf"]
                    if not np.isscalar(mu_conf) and i < len(mu_conf):
                        if np.isscalar(mu_conf[i]):
                ...
                if i < len(sigma_values):
                    sensitivity_indices[f"sigma_{name}"] = float(sigma_values[i])
                if "sigma_conf" in morris_indices:
                    sigma_conf = morris_indices["sigma_conf"]
                    if not np.isscalar(sigma_conf) and i < len(sigma_conf):
                        if np.isscalar(sigma_conf[i]):
                            ...
                            confidence_intervals[f"sigma_{name}"] = (
                                float(sigma_conf[i][0]),
                                float(sigma_conf[i][1]),
                            )

                # Elementary effects mean of absolute values (mu_star)
                if i < len(mu_star_values):
                    sensitivity_indices[f"mu_star_{name}"] = float(mu_star_values[i])
                if "mu_star_conf" in morris_indices:
                    mu_star_conf = morris_indices["mu_star_conf"]
                    if not np.isscalar(mu_star_conf) and i < len(mu_star_conf):
                        if np.isscalar(mu_star_conf[i]):
                            confidence_intervals[f"mu_star_{name}"] = (
                                float(mu_star_conf[i]),
                                float(mu_star_conf[i]),
                            )
                        else:
                            confidence_intervals[f"mu_star_{name}"] = (
                                float(mu_star_conf[i][0]),
                                float(mu_star_conf[i][1]),
                            )
