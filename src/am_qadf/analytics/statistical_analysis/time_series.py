...
class TimeSeriesAnalyzer:
    ...
    def analyze_trend(self, time_series: pd.Series, method: str = None) -> TimeSeriesResult:
        """
    ...
    def analyze_seasonality(self, time_series: pd.Series) -> TimeSeriesResult:
        """
    ...
    def analyze_forecasting(self, time_series: pd.Series, forecast_horizon: int = None) -> TimeSeriesResult:
        """
    ...
    def forecast(self, time_series: pd.Series, horizon: int = None) -> TimeSeriesResult:
        """
        Forecast future values of time series.

        Args:
            time_series: Input time series data
            horizon: Number of periods to forecast (optional)

        Returns:
            TimeSeriesResult: Forecasting results
        """
        return self.analyze_forecasting(time_series, forecast_horizon=horizon)

    def comprehensive_analysis(self, time_series: pd.Series) -> TimeSeriesResult:
        """
        Perform comprehensive time series analysis (trend, seasonality, and forecasting).

        Args:
            time_series: Input time series data

        Returns:
            TimeSeriesResult: Comprehensive analysis results
        """
        # Analyze trend
        trend_result = self.analyze_trend(time_series, method="linear")

        # Analyze seasonality
        seasonality_result = self.analyze_seasonality(time_series)

        # Perform forecasting
        forecast_result = self.analyze_forecasting(time_series)

        # Combine results
        combined_result = TimeSeriesResult(
            success=True,
            method="Comprehensive",
            time_series_data=time_series,
            trend_analysis=trend_result.trend_analysis if trend_result.success else {},
            seasonality_analysis=(seasonality_result.seasonality_analysis if seasonality_result.success else {}),
            forecasting_results=(forecast_result.forecasting_results if forecast_result.success else {}),
            analysis_time=trend_result.analysis_time + seasonality_result.analysis_time + forecast_result.analysis_time,
        )
    ...
    def _detect_seasonality(self, y: np.ndarray) -> Dict[str, Any]:
        ...
        valid_periods = periods[(periods >= self.config.min_periods) & (periods <= self.config.max_periods)]
        ...
        # Get primary period if available
        primary_period = float(valid_periods[0]) if len(valid_periods) > 0 else None

        seasonality_result = {
            "is_seasonal": is_seasonal,
            "has_seasonality": is_seasonal,  # Alias for test compatibility
            "seasonality_strength": seasonality_strength,
            "dominant_periods": valid_periods,
            "period": primary_period,  # Primary period for test compatibility
