...
class RegressionAnalyzer:
    ...
    def analyze_linear_regression(self, X: pd.DataFrame, y: pd.Series, feature_names: List[str] = None) -> RegressionResult:
        """
        ...
        try:
            ...
            confidence_intervals = self._calculate_confidence_intervals(X_scaled, y_data, predictions, residuals)
    ...
    def _calculate_confidence_intervals(
        ...
        se_mean = np.sqrt(
            mse * (1 / n + np.sum((X - np.mean(X, axis=0)) ** 2, axis=1) / np.sum((X - np.mean(X, axis=0)) ** 2))
        )
    ...
    def get_cached_result(
        self, method: str, feature_names: List[str] = None, target_name: str = None
    ...
    def analyze_ridge_regression(
        ...
        Returns:
            RegressionResult: Ridge regression analysis results
        """
        original_alpha = self.config.regularization_alpha
        self.config.regularization_alpha = alpha
        try:
            result = self.analyze_regularized_regression(X, y, feature_names, model_type="ridge")
            result.method = "RidgeRegression"
            return result
        finally:
            self.config.regularization_alpha = original_alpha
    ...
    def analyze_lasso_regression(
        ...
        Returns:
            RegressionResult: Lasso regression analysis results
        """
        original_alpha = self.config.regularization_alpha
        self.config.regularization_alpha = alpha
        try:
            result = self.analyze_regularized_regression(X, y, feature_names, model_type="lasso")
            result.method = "LassoRegression"
            return result
        finally:
            self.config.regularization_alpha = original_alpha

    def analyze_elastic_net_regression(
        self,
        X: pd.DataFrame,
        y: pd.Series,
        feature_names: List[str] = None,
        ...
        """
        Perform elastic net regression analysis.
        ...
        Args:
            ...
            feature_names: List of feature names (optional)
            alpha: Regularization strength
            l1_ratio: Mixing parameter for L1/L2 regularization

        Returns:
            RegressionResult: Elastic net regression analysis results
        """
        original_alpha = self.config.regularization_alpha
        original_l1_ratio = self.config.l1_ratio
        self.config.regularization_alpha = alpha
        self.config.l1_ratio = l1_ratio
        try:
            result = self.analyze_regularized_regression(X, y, feature_names, model_type="elastic_net")
            result.method = "ElasticNetRegression"
            return result
        finally:
            self.config.regularization_alpha = original_alpha
            self.config.l1_ratio = original_l1_ratio
