...
class NonparametricAnalyzer:
    ...
    def analyze_kernel_density(self, data: pd.Series, data_name: str = None) -> NonparametricResult:
        """
        ...
        try:
            ...
            kde = KernelDensity(bandwidth=self.config.kde_bandwidth, kernel=self.config.kde_kernel)
    ...
    def analyze_spearman_correlation(
        ...
        Returns:
            NonparametricResult: Spearman correlation analysis results
        """
        try:
            start_time = datetime.now()
            ...
            result = NonparametricResult(
                success=True,
                method="SpearmanCorrelation",
                data_names=[data_name1, data_name2],
                analysis_results={"spearman_correlation": corr},
                test_statistics={"spearman_correlation": corr},
                p_values={"spearman_p_value": p_value},
                confidence_intervals={},
                analysis_time=analysis_time,
            )

            # Cache result
            self._cache_result("spearman_correlation", result)

            logger.info(f"Spearman correlation analysis completed: {analysis_time:.2f}s")
            return result

        except Exception as e:
            logger.error(f"Error in Spearman correlation analysis: {e}")
            return NonparametricResult(
                success=False,
                method="SpearmanCorrelation",
                data_names=[data_name1 or "data1", data_name2 or "data2"],
                analysis_results={},
                test_statistics={},
                p_values={},
                confidence_intervals={},
    ...
    def analyze_kendall_tau(
        ...
        Returns:
            NonparametricResult: Kendall's tau correlation analysis results
        """
        try:
            start_time = datetime.now()
            ...
            result = NonparametricResult(
                ...
                method="KendallTau",
                data_names=[data_name1, data_name2],
                analysis_results={"kendall_tau": tau},
                test_statistics={"kendall_tau": tau},
                p_values={"kendall_p_value": p_value},
                confidence_intervals={},
                analysis_time=analysis_time,
            )

            # Cache result
            self._cache_result("kendall_tau", result)

            logger.info(f"Kendall's tau analysis completed: {analysis_time:.2f}s")
            return result

        except Exception as e:
            logger.error(f"Error in Kendall's tau analysis: {e}")
            return NonparametricResult(
                success=False,
                method="KendallTau",
                data_names=[data_name1 or "data1", data_name2 or "data2"],
    ...
    def analyze_correlation(
        ...
        try:
            ...
            # Calculate confidence intervals
            confidence_intervals = self._calculate_correlation_confidence_intervals(spearman_corr, kendall_tau, len(y1))
    ...
    def _calculate_confidence_intervals(self, y1: np.ndarray, y2: np.ndarray = None) -> Dict[str, Tuple[float, float]]:
    ...
    def _calculate_correlation_confidence_intervals(
        self, spearman_corr: float, kendall_tau: float, n: int
        ...
        confidence_intervals["spearman_confidence_interval"] = (
            ci_lower_spearman,
            ci_upper_spearman,
        )
        ...
        confidence_intervals["kendall_confidence_interval"] = (
            ci_lower_kendall,
            ci_upper_kendall,
        )
