...
class ParameterAnalyzer:
    ...
    def analyze_parameter_interactions(
        ...
        try:
            ...
            # Handle output_name alias for target_variable
            if output_name is not None:
                target_variable = output_name
            ...
            # Return the interactions dict directly for compatibility with tests
    ...
    def analyze_parameter_importance(
        ...
        """
        Analyze parameter importance based on correlation with output.

        Args:
            process_data: DataFrame containing process data
            parameter_names: List of parameter names (optional)
            output_name: Name of the output/target variable
        ...
        try:
            if output_name is None:
                output_name = process_data.columns[-1]  # Use last column as output

            if parameter_names is None:
                parameter_names = [col for col in process_data.columns if col != output_name]

            # Prepare data
            X = process_data[parameter_names].values
            y = process_data[output_name].values
            ...
            logger.error(f"Error in parameter importance analysis: {e}")
            return {}
...
class ProcessParameterOptimizer(ParameterAnalyzer):
    ...
    def optimize_process_parameters(
        ...
        Returns:
            ParameterAnalysisResult: Optimization results
        """
        return self.analyze_parameter_optimization(objective_function, parameter_bounds, parameter_names)
