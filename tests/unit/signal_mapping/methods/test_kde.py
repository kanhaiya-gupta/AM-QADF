...
class TestGaussianKDEInterpolation:
    ...
    def test_kde_interpolation_creation_default(self):
        """Test creating GaussianKDEInterpolation with default parameters."""
    ...
    @pytest.mark.unit
    def test_estimate_bandwidth(self, interpolation_method):
        ...
        bandwidth = interpolation_method._estimate_bandwidth(points)

        assert bandwidth > 0
        assert bandwidth >= 0.1  # Minimum bandwidth

    @pytest.mark.unit
    def test_estimate_bandwidth_single_point(self, interpolation_method):
        """Test bandwidth estimation with single point."""
        points = np.array([[5.0, 5.0, 5.0]])

        bandwidth = interpolation_method._estimate_bandwidth(points)

        assert bandwidth == 1.0  # Default for n < 2

    @pytest.mark.unit
    def test_estimate_bandwidth_empty(self, interpolation_method):
        """Test bandwidth estimation with empty points."""
    ...
    @pytest.mark.unit
    def test_interpolate_auto_bandwidth(self, voxel_grid):
        """Test interpolation with auto-estimated bandwidth."""
        method = GaussianKDEInterpolation(bandwidth=None)  # Auto-estimate
        ...
        assert result is voxel_grid
        assert method.bandwidth is not None
        assert method.bandwidth > 0
    ...
    def test_interpolate_bandwidth_effect(self):
        ...
        assert len(result_small.voxels) > 0
        assert len(result_large.voxels) > 0
