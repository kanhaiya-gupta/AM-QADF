"""
Unit tests for sequential execution.

Tests for sequential interpolation execution and hatching path interpolation.
"""
...
from am_qadf.signal_mapping.execution.sequential import (
    ...
    _nearest_neighbor_interpolation_legacy,
...
class TestInterpolateToVoxels:
    ...
    @pytest.mark.unit
    def test_interpolate_to_voxels_use_vectorized_false(self, voxel_grid):
        """Test interpolate_to_voxels with use_vectorized=False (legacy)."""
        points = np.array([[5.0, 5.0, 5.0], [6.0, 6.0, 6.0]])
        signals = {"power": np.array([200.0, 250.0])}

        result = interpolate_to_voxels(points, signals, voxel_grid, method="nearest", use_vectorized=False)

        assert result is voxel_grid
        assert len(voxel_grid.voxels) > 0

    @pytest.mark.unit
    def test_interpolate_to_voxels_use_vectorized_false_invalid_method(self, voxel_grid):
        """Test interpolate_to_voxels with use_vectorized=False and non-nearest method."""
        points = np.array([[5.0, 5.0, 5.0]])
        signals = {"power": np.array([200.0])}

        with pytest.raises(NotImplementedError, match="Non-vectorized interpolation method"):
            interpolate_to_voxels(points, signals, voxel_grid, method="linear", use_vectorized=False)
...
class TestNearestNeighborInterpolationLegacy:
    """Test suite for _nearest_neighbor_interpolation_legacy function."""

    @pytest.mark.unit
    def test_legacy_interpolation_basic(self):
        """Test legacy nearest neighbor interpolation."""
        ...
        points = np.array([[5.0, 5.0, 5.0], [5.1, 5.1, 5.1], [6.0, 6.0, 6.0]])
        signals = {
            "power": np.array([200.0, 250.0, 300.0]),
            "speed": np.array([100.0, 150.0, 200.0]),
        }

        result = _nearest_neighbor_interpolation_legacy(points, signals, voxel_grid)

        assert result is voxel_grid
        assert len(voxel_grid.voxels) > 0

    @pytest.mark.unit
    def test_legacy_interpolation_empty_points(self):
        """Test legacy interpolation with empty points."""
...
class TestInterpolateHatchingPaths:
    ...
    def test_interpolate_hatching_paths_short_path(self, voxel_grid):
        ...
        # Should skip paths with < 2 points
        assert result is voxel_grid
    ...
    def test_interpolate_hatching_paths_custom_points_per_mm(self, voxel_grid):
        ...
        assert result is voxel_grid
        # Higher points_per_mm should create more sampled points
