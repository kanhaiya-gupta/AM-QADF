...
import pytest
import numpy as np
from am_qadf.quality.data_quality import (
    DataQualityMetrics,
    DataQualityAnalyzer,
)
...
class TestDataQualityAnalyzer:
    ...
    @pytest.mark.unit
    def test_data_quality_analyzer_creation(self, analyzer):
    ...
    @pytest.mark.unit
    def test_calculate_completeness_empty_signals(self, analyzer):
        """Test calculating completeness with empty signals."""
        voxel_data = MockVoxelData({}, dims=(10, 10, 10))

        completeness = analyzer.calculate_completeness(voxel_data, [])

        assert completeness == 0.0

    @pytest.mark.unit
    def test_calculate_completeness_no_available_signals(self, analyzer):
        """Test calculating completeness when no available_signals attribute."""

        class NoSignalsVoxelData:
            pass

        voxel_data = NoSignalsVoxelData()
    ...
    @pytest.mark.unit
    def test_calculate_spatial_coverage_empty_signals(self, analyzer):
        """Test calculating spatial coverage with empty signals."""
        voxel_data = MockVoxelData({}, dims=(10, 10, 10))

        coverage = analyzer.calculate_spatial_coverage(voxel_data, [])

        assert coverage == 0.0

    @pytest.mark.unit
    def test_calculate_spatial_coverage_no_bbox(self, analyzer):
        """Test calculating spatial coverage without bbox."""

        class NoBBoxVoxelData:
            available_signals = ["signal1"]
            dims = (10, 10, 10)

        voxel_data = NoBBoxVoxelData()
    ...
    @pytest.mark.unit
    def test_calculate_temporal_coverage_with_layer_range(self, analyzer):
        """Test calculating temporal coverage with layer range."""
        signals = {"signal1": np.ones((10, 10, 10)), "signal2": np.ones((10, 10, 10))}
        voxel_data = MockVoxelData(signals, dims=(10, 10, 10))

        coverage = analyzer.calculate_temporal_coverage(voxel_data, layer_range=(0, 5))

        assert 0.0 <= coverage <= 1.0

    @pytest.mark.unit
    def test_calculate_temporal_coverage_no_dims(self, analyzer):
        """Test calculating temporal coverage without dims."""

        class NoDimsVoxelData:
            available_signals = []

        voxel_data = NoDimsVoxelData()
    ...
    @pytest.mark.unit
    def test_calculate_consistency_single_signal(self, analyzer):
        """Test calculating consistency with single signal."""
        signals = {"signal1": np.array([1.0, 2.0, 3.0])}
        voxel_data = MockVoxelData(signals, dims=(3, 1, 1))

        consistency = analyzer.calculate_consistency(voxel_data, ["signal1"])

        assert consistency == 1.0  # Single signal is always consistent

    @pytest.mark.unit
    def test_calculate_consistency_less_than_two(self, analyzer):
        """Test calculating consistency with less than two signals."""
        signals = {"signal1": np.array([1.0, 2.0, 3.0])}
        voxel_data = MockVoxelData(signals, dims=(3, 1, 1))
    ...
    def test_identify_missing_regions(self, analyzer):
        ...
        missing_regions = analyzer.identify_missing_regions(voxel_data, ["signal1"], min_region_size=10)

        assert isinstance(missing_regions, list)
        assert len(missing_regions) >= 0  # May or may not find region depending on size

    @pytest.mark.unit
    def test_identify_missing_regions_empty_signals(self, analyzer):
        """Test identifying missing regions with empty signals."""
        voxel_data = MockVoxelData({}, dims=(10, 10, 10))

        missing_regions = analyzer.identify_missing_regions(voxel_data, [])
    ...
    @pytest.mark.unit
    def test_assess_quality_with_layer_range(self, analyzer):
        """Test assessing quality with layer range."""
        signals = {"signal1": np.ones((10, 10, 10)), "signal2": np.ones((10, 10, 10))}
        voxel_data = MockVoxelData(signals, dims=(10, 10, 10))

        metrics = analyzer.assess_quality(voxel_data, ["signal1", "signal2"], layer_range=(0, 5))

        assert isinstance(metrics, DataQualityMetrics)
        assert 0.0 <= metrics.coverage_temporal <= 1.0

    @pytest.mark.unit
    def test_assess_quality_empty_signals(self, analyzer):
        """Test assessing quality with empty signals."""
        voxel_data = MockVoxelData({}, dims=(10, 10, 10))

        metrics = analyzer.assess_quality(voxel_data, [])
