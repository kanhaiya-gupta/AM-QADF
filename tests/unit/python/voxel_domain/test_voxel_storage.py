...
VoxelGridStorage requires am_qadf_native (VDBWriter, OpenVDBReader, numpy_to_openvdb, openvdb_to_numpy).
"""
...
class MockMongoClient:
    ...
    def store_file(self, data, filename=None, bucket_name=None, metadata=None):
        """Store a file in GridFS (mock). Accepts bucket_name for compatibility with voxel_storage."""
    ...
    def get_file(self, file_id, bucket_name=None):
        """Get a file from GridFS (mock). Accepts bucket_name for OpenVDB path compatibility."""
...
@pytest.mark.skipif(
    not VOXELIZATION_AVAILABLE or VoxelGridStorage is None,
    reason="VoxelGridStorage or am_qadf_native/voxelization not available",
)
class TestVoxelGridStorage:
    ...
    @pytest.mark.unit
    def test_save_voxel_grid_stores_signals(self, storage, voxel_grid, mongo_client):
        """Test that voxel grid (including signals) is stored via OpenVDB."""
        storage.save_voxel_grid(model_id="test_model", grid_name="test_grid", voxel_grid=voxel_grid)
        # OpenVDB path: one .vdb file stored; document has _openvdb_file reference
        collection = mongo_client.get_collection(storage.collection_name)
        doc = collection.find_one({"model_id": "test_model", "grid_name": "test_grid"})
        assert doc is not None
        refs = doc.get("signal_references", {})
        assert "_openvdb_file" in refs
        # Stored file should be in GridFS mock
        assert refs["_openvdb_file"] in mongo_client._files

    @pytest.mark.unit
    def test_save_voxel_grid_signal_fallback(self, storage, voxel_grid):
        """Test that RuntimeError is raised when OpenVDB storage fails."""
        with patch.object(storage, "_store_voxel_grid_openvdb", return_value=None):
            with pytest.raises(RuntimeError, match="Failed to store voxel grid"):
                storage.save_voxel_grid(model_id="test_model", grid_name="test_grid", voxel_grid=voxel_grid)
    ...
    def test_load_voxel_grid_loads_signals(self, storage, voxel_grid):
        """Test that signals are loaded from OpenVDB."""
        grid_id = storage.save_voxel_grid(model_id="test_model", grid_name="test_grid", voxel_grid=voxel_grid)

        loaded = storage.load_voxel_grid(grid_id)
        assert loaded is not None
        assert "signal_arrays" in loaded
        # OpenVDB path loads all signals from the .vdb file
        assert len(loaded["signal_arrays"]) == len(voxel_grid.available_signals)
        for name in voxel_grid.available_signals:
            assert name in loaded["signal_arrays"]
            assert isinstance(loaded["signal_arrays"][name], np.ndarray)
