...
MEMORY_BASELINES = {
    ...
    "signal_mapping_100k": 200.0,  # 100K points (sequential, C++ signal mapping)
}
...
class TestMemoryRegression:
    ...
    @pytest.mark.regression
    def test_signal_mapping_memory_100k(self, memory_baselines):
        """Test memory usage for signal mapping (100K points) - sequential C++ execution."""
        try:
            from am_qadf.signal_mapping.execution.sequential import (
                interpolate_to_voxels,
            )
            ...
            peak_mb = bytes_to_mb(peak)
            baseline_mb = memory_baselines.get("signal_mapping_100k")

            is_regressed, increase, message = check_memory_regression("signal_mapping_100k", peak_mb, baseline_mb)
