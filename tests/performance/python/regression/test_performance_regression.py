...
# Note: Sequential processing is limited to 200K points maximum for these tests.
...
PERFORMANCE_BASELINES = {
    ...
    "interpolation_idw": 3.0,  # IDW interpolation (smaller grid: 15K pts, res=2 → 50³ voxels)
    "signal_mapping_100k": 10.0,  # 100K points (sequential, C++ signal mapping)
}
...
class TestPerformanceRegression:
    ...
    def test_voxel_fusion_small_regression(self, baselines):
        ...
        for i in range(3):
            ...
            grid.available_signals = {"temperature"}
            grids.append(grid)

        fusion = VoxelFusion()

        start_time = time.time()
        fused = fusion.fuse_grids(grids, strategy="weighted_average")
        execution_time = time.time() - start_time
    ...
    def test_interpolation_idw_regression(self, baselines):
        """Test IDW interpolation performance regression - C++ IDW with smaller grid for speed."""
        try:
            from am_qadf.signal_mapping.execution.sequential import (
                interpolate_to_voxels,
            )

            # IDW is O(voxels × k_neighbors): use smaller grid (50³ voxels, 15K pts) so test stays fast
            points, signals = create_large_point_cloud(15000)
            grid = create_voxel_grid(bbox_size=100.0, resolution=2.0)
            ...
            # Auto-update baseline if current time is slower but within 5x (first run or problem size changed)
            if baseline_time and execution_time > baseline_time * 1.1 and execution_time < baseline_time * 5.0:
                baselines["interpolation_idw"] = execution_time
                save_baselines(baselines)
    ...
    @pytest.mark.regression
    def test_signal_mapping_100k_regression(self, baselines):
        """Test signal mapping performance regression (100K points) - sequential C++ execution."""
        try:
            from am_qadf.signal_mapping.execution.sequential import (
                interpolate_to_voxels,
            )
            ...
            baseline_time = baselines.get("signal_mapping_100k")

            # Auto-update baseline if current time is slower but within 3x (first run or performance changed)
            if baseline_time and execution_time > baseline_time * 1.1 and execution_time < baseline_time * 3.0:
                baselines["signal_mapping_100k"] = execution_time
                save_baselines(baselines)
                baseline_time = execution_time  # Use updated baseline for check

            is_regressed, degradation, message = check_performance_regression(
                "signal_mapping_100k", execution_time, baseline_time
            )

            if is_regressed:
                pytest.fail(f"Performance regression: {message}")
        except ImportError:
            pytest.skip("Signal mapping (interpolation) module not available")
